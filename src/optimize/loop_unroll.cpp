#include "../../include/optimize/loop_unroll.h"
...
void add_scope_id(IRFunction* func) {
  for (auto& bb : func->bb_list_) {
    for (auto& ir : bb->ir_list_) {
      ...
      for (auto& opn : tmp) {
        if (opn->type_ == ir::Opn::Type::Var) opn->name_ += "_#" + to_string(opn->scope_id_);
      }
    }
  }
}
...
vector<pair<IRBasicBlock*, IRBasicBlock*> > find_back(vector<IRBasicBlock*> bb_list) {
  ...
  for (int i = 1; i < bb_list.size(); i++) {
    for (int j = 0; j < bb_list.size(); j++) dom[bb_list[i]].push_back(bb_list[j]);
  }
  bool flag = true;
  while (flag) {
    flag = false;
  ...
  for (auto& bb : bb_list) {
    for (auto& suc : bb->succ_) {
      if (have(suc, dom[bb])) back.push_back(make_pair(bb, suc));
    }
  }
  return back;
}
...
vector<ir::Opn*> find_basicvar(ir::Opn* var, IRBasicBlock* bb) {
  ...
  for (auto& ir : rev) {
    if (ir->op_ == ir::IR::OpKind::LABEL || ir->op_ == ir::IR::OpKind::PARAM || ir->op_ == ir::IR::OpKind::CALL ||
        ir->op_ == ir::IR::OpKind::RET || ir->op_ == ir::IR::OpKind::GOTO)
      continue;
    auto res = &ir->res_;
    if (have1(res, basic)) basic.push_back(&ir->opn1_), basic.push_back(&ir->opn2_);
  }
  ...
  while (it != basic.end()) {
    ...
    if (count(name.begin(), name.end(), '-') == 1)
      it = basic.erase(it);  // 删除临时变量，临时变量形如temp-x
    else
      it++;
  }
  return basic;
...
void LoopUnroll::Run() {
  ...
  for (int func_id = 0; func_id < irmodule->func_list_.size(); func_id++) {
    ...
    for (int i = 0; i < bb_list.size(); i++) bb_id[bb_list[i]] = i, id_bb[i] = bb_list[i];
    ...
    for (auto& edge : back) {
      ...
      bool flag = true;
      for (auto basic : basic1)
        flag &= (basic->type_ != ir::Opn::Type::Array);  // 只要basic1和2里有1个涉及到数组就不优化
      ...
      if (!(change1.size() == 2 && change2.size() == 0) && !(change1.size() == 0 && change2.size() == 2))
        continue;  // 这里后续可以改，这里限制死了，i只能是i=i op c,n不能有定值
      ...
      for (int i = 0; i < loop[1]->ir_list_.size(); i++) {
        if (loop[1]->ir_list_[i]->res_.name_ == op1->name_) {
          if (i && loop[1]->ir_list_[i - 1]->opn1_.name_ == change1[0]->name_ &&
              loop[1]->ir_list_[i - 1]->opn2_.name_ == change1[1]->name_) {
      ...
      // i+c的那个c
      ir::Opn* c;
      if (cal_ir->opn1_.name_ == op1->name_)
        c = &cal_ir->opn2_;
      else
        c = &cal_ir->opn1_;
      ...
      if (loop[0]->pred_.size() == 2)  // loop[1]也是loop[0]的pred之一，则要求循环外只能1个bb到这里，就是2
      {
        ...
        if (loop[0]->pred_[0] == loop[1])
          pred_bb = loop[0]->pred_[1];
        else
          pred_bb = loop[0]->pred_[0];

        vector<ir::IR*> rev(pred_bb->ir_list_);
        reverse(rev.begin(), rev.end());
        ir::IR *def_i = nullptr, *def_n = nullptr;
        for (auto ir : rev) {
          if (def_i == nullptr && ir->res_.name_ == op1->name_) def_i = ir;
          if (def_n == nullptr && ir->res_.name_ == op2->name_) def_n = ir;
        }
      ...
      if (determined) {
        ...
        ir::Opn* new_temp1 =
            new ir::Opn(ir::Opn::Type::Var, "temp-" + to_string(++temp) + "_#" + to_string(scope_id), scope_id);
